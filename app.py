...
# Anchor all asset paths to this file's own folder, not the process's
# working directory (which varies across local runs / Streamlit Cloud).
...
def flatten(html: str) -> str:
    ...
    st.markdown() runs content through a Markdown parser before rendering
    HTML, and CommonMark treats any line indented 4+ spaces as a code
    block. Our HTML is built from nested, indented Python loops/f-strings,
    so without this, large chunks render as literal code instead of as
    markup.
    """
...
if not LOGO_PATH.exists():
    st.error(
        f"Logo not found at `{LOGO_PATH}`. Make sure `assets/logo.png` is "
        "committed to your repo alongside app.py."
    )
    st.stop()
...
st.markdown(flatten(CSS), unsafe_allow_html=True)
...
st.markdown(flatten(page_html), unsafe_allow_html=True)
